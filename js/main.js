...
(function voiceUI(){
  ...
  /* 声の一覧(2026-09-06: プルダウンの箱を廃し常時表示の一覧へ。選択中の行だけ波形を出す) */
  const list=document.getElementById('vdropList');
  if(list){
    list.addEventListener('click',e=>{const o=e.target.closest('.vd-opt');if(!o)return;
      list.querySelectorAll('.vd-opt').forEach(x=>{const on=x===o;x.classList.toggle('sel',on);x.setAttribute('aria-selected',on?'true':'false');});});
  }
...
"手元で生まれた素材":"在手邊生成的素材","完成作品の上映時間":"完成作品的總片長","184分":"184分鐘",
...
"ラジオ":"廣播","解説":"解說",
"RINKAのオフレコ #04":"RINKA的悄悄話 #04","重いものだけが、残った——『重力』の制作を、KANADEと順位をつけて振り返る。":"只有重的，留了下來——和 KANADE 一起排名次，回顧《重力》的製作。","RINKAのオフレコ #03":"RINKA的悄悄話 #03","跳ねていなかったのは、わたしじゃなかった——初ゲストのKANADEと、犯人捜しをする制作裏話。":"不會跳的，其實不是我——和第一位來賓 KANADE 一起，聊聊找出真兇的製作幕後。","RINKAのオフレコ #02":"RINKA的悄悄話 #02",
"データは出ないのに、わたしの失敗は出ていく——監督のいないところで話す、制作裏話。":"資料不出門，我的失誤卻天天出門——趁導演不在，聊聊製作的幕後。",
"海煙":"海煙","朝の海のMV ・ 3分42秒":"晨海的MV ・ 3分42秒",
